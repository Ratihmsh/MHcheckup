...
<div class="sidebar">
    ...
    <div class="sidebar-menu">
        ...
        <a href="{{ route('admin.peserta.list') }}" class="sidebar-item active">
...
<div class="main-content">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('admin.peserta.cetak', $peserta->id) }}" target="_blank" class="btn-cetak">
        ...
        <div class="col-lg-5">
            ...
            <div class="card mb-4">
                <div class="card-body p-4">
                    ...
                    <div class="info-row"><span class="info-label">Nama</span><span class="info-value fw-bold">{{ $peserta->nama }}</span></div>
                    <div class="info-row"><span class="info-label">Jenis Kelamin</span><span class="info-value">{{ $peserta->jenis_kelamin }}</span></div>
                    <div class="info-row"><span class="info-label">Tanggal Lahir</span><span class="info-value">{{ \Carbon\Carbon::parse($peserta->tanggal_lahir)->format('d/m/Y') }}</span></div>
                    <div class="info-row"><span class="info-label">Usia</span><span class="info-value">{{ $peserta->usia }}</span></div>
                    <div class="info-row"><span class="info-label">Pendidikan</span><span class="info-value">{{ $peserta->pendidikan ?? '-' }}</span></div>
                    <div class="info-row"><span class="info-label">Pekerjaan</span><span class="info-value">{{ $peserta->pekerjaan ?? '-' }}</span></div>
                    <div class="info-row"><span class="info-label">Status Kawin</span><span class="info-value">{{ $peserta->status_perkawinan ?? '-' }}</span></div>
                    <div class="info-row"><span class="info-label">No. Telepon</span><span class="info-value">{{ $peserta->no_telp ?? '-' }}</span></div>
                    <div class="info-row"><span class="info-label">Alamat</span><span class="info-value">{{ $peserta->alamat ?? '-' }}</span></div>
                    <div class="info-row"><span class="info-label">Tgl Pemeriksaan</span><span class="info-value">{{ \Carbon\Carbon::parse($peserta->tanggal_pemeriksaan)->format('d/m/Y') }}</span></div>
                </div>
            </div>
            ...
            <div class="card mb-4">
                <div class="card-body p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="text-dark" style="font-size:0.88rem; line-height:1.6;">{{ $peserta->permasalahan ?? '-' }}</div>
                    </div>
                    <div class="mb-3">
                        <div class="info-label mb-1">Yang Dirasakan</div>
                        <div class="text-dark" style="font-size:0.88rem; line-height:1.6;">{{ $peserta->yang_dirasakan ?? '-' }}</div>
                    </div>
                    <div>
                        <div class="info-label mb-1">Harapan</div>
                        <div class="text-dark" style="font-size:0.88rem; line-height:1.6;">{{ $peserta->harapan ?? '-' }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body p-4">
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-4">
                            <div class="skor-mini dep">
                                ...
                                <div class="angka">{{ $hasil->skor_depresi }}</div>
                                <span class="badge-kategori badge-{{ str_replace(' ','-',$hasil->kategori_depresi) }} mt-1">
                        ...
                        <div class="col-4">
                            <div class="skor-mini kec">
                                ...
                                <div class="angka">{{ $hasil->skor_kecemasan }}</div>
                                <span class="badge-kategori badge-{{ str_replace(' ','-',$hasil->kategori_kecemasan) }} mt-1">
                        ...
                        <div class="col-4">
                            <div class="skor-mini str">
                                ...
                                <div class="angka">{{ $hasil->skor_stres }}</div>
                                <span class="badge-kategori badge-{{ str_replace(' ','-',$hasil->kategori_stres) }} mt-1">
                    ...
                    {{-- HAPUS --}}
                    <form action="{{ route('admin.peserta.hapus', $peserta->id) }}" method="POST"
                        onsubmit="return confirm('Yakin hapus data {{ $peserta->nama }}? Data tidak bisa dikembalikan.')">
                        @csrf
                        @method('DELETE')
        ...
        <div class="col-lg-7">
            ...
            <div class="card mb-4">
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('admin.peserta.update-laporan', $peserta->id) }}" method="POST" id="formLaporan">
                            ...
                            @foreach($fields as $key => $label)
                                <div class="mb-3">
                                    ...
                                    <textarea
                                        ...
                                        class="form-control"
                                        rows="3"
                                        placeholder="Masukkan dinamika {{ strtolower($label) }}...">{{ $dinamika[$key] ?? '' }}</textarea>
                                </div>
                            @endforeach
                            ...
                            @foreach($fields as $key => $label)
                                <div class="mb-3">
                                    ...
                                    <textarea
                                        ...
                                        class="form-control"
                                        rows="3"
                                        placeholder="Masukkan saran {{ strtolower($label) }}...">{{ $saran[$key] ?? '' }}</textarea>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <table class="table tabel-jawaban mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center fw-bold">{{ $nilaiJawaban }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    function generateAi() {
        ...
        fetch('{{ route("admin.generate.ai", $peserta->id) }}', {
            method: 'POST',
            headers: {
