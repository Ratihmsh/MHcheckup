                ...
                <table class="table align-middle mb-0" style="font-size: 0.9rem;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-4">
                                @if($b->gambar)
                                    <img src="{{ asset($b->gambar) }}" class="berita-thumb">
                                @else
                            ...
                            <td>{{ \Carbon\Carbon::parse($b->tanggal_publish)->translatedFormat('d/m/Y') }}</td>
                            <td class="text-secondary">
                                {{ Str::limit(strip_tags($b->konten), 80) }}
                            </td>
                            <td class="text-end pe-4">
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="{{ route('admin.berita.edit', $b->id) }}" class="btn-edit">
                                        <i class="bi bi-pencil-fill"></i> Edit
                                    </a>
                                    <form action="{{ route('admin.berita.destroy', $b->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus berita ini?')">
                                        @csrf
                                        @method('DELETE')
