...
<div class="sidebar">
    <div class="sidebar-brand">
        <img src="{{ asset('images/logo.jpg') }}" alt="Ariva" style="height:32px; object-fit:contain; background:#fff; border-radius:6px; padding:2px 4px;">
...
<div class="main-content">
    ...
    <div class="summary-bar">
        <div class="summary-item">
            ...
            <strong>{{ $totalSemua }} peserta</strong>
        </div>
        <div class="summary-item">
        ...
        <div class="summary-item">
            ...
            <strong>{{ $totalPerhatian }}</strong>
        </div>
        <div class="ms-auto">
            <small class="text-secondary">Menampilkan {{ $peserta->total() }} hasil</small>
        </div>
    </div>
    ...
    <div class="table-card">
        <div class="d-flex align-items-center justify-content-between p-4 border-bottom flex-wrap gap-3">
            ...
            <form action="{{ route('admin.peserta.list') }}" method="GET" class="d-flex gap-2 flex-wrap align-items-center">
                <input type="text" name="search" class="search-box" placeholder="Cari nama..." value="{{ $search }}" style="min-width:180px;">
                <input type="date" name="tanggal" class="filter-box" value="{{ request('tanggal') }}">
                <select name="kategori" class="filter-box">
                    <option value="">Semua Kategori</option>
                    @foreach(['Normal','Ringan','Sedang','Tinggi','Sangat Tinggi'] as $k)
                ...
                @if($search || request('tanggal') || request('kategori'))
                <a href="{{ route('admin.peserta.list') }}" class="btn btn-outline-secondary btn-sm px-3" style="border-radius:10px;">
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-secondary">{{ $peserta->firstItem() + $index }}</td>
                        <td>
                            <div class="fw-bold">{{ $p->nama }}</div>
                            <small class="text-secondary">{{ $p->jenis_kelamin }}, {{ $p->usia }} thn</small>
                        </td>
                        <td>{{ \Carbon\Carbon::parse($p->tanggal_pemeriksaan)->format('d/m/Y') }}</td>
                        <td class="text-center">
                            @if($p->kategori_depresi)
                                <span class="badge-kategori badge-{{ str_replace(' ','-',$p->kategori_depresi) }}">{{ $p->kategori_depresi }}</span>
                                <div class="text-secondary" style="font-size:0.75rem;">{{ $p->skor_depresi }}</div>
                            @else <span class="text-secondary">-</span> @endif
                        </td>
                        <td class="text-center">
                            @if($p->kategori_kecemasan)
                                <span class="badge-kategori badge-{{ str_replace(' ','-',$p->kategori_kecemasan) }}">{{ $p->kategori_kecemasan }}</span>
                                <div class="text-secondary" style="font-size:0.75rem;">{{ $p->skor_kecemasan }}</div>
                            @else <span class="text-secondary">-</span> @endif
                        </td>
                        <td class="text-center">
                            @if($p->kategori_stres)
                                <span class="badge-kategori badge-{{ str_replace(' ','-',$p->kategori_stres) }}">{{ $p->kategori_stres }}</span>
                                <div class="text-secondary" style="font-size:0.75rem;">{{ $p->skor_stres }}</div>
                            @else <span class="text-secondary">-</span> @endif
                        </td>
                        <td class="text-center">
                            <div class="d-flex gap-2 justify-content-center">
                                <a href="{{ route('admin.peserta.detail', $p->id) }}" class="btn-detail">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <form action="{{ route('admin.peserta.hapus', $p->id) }}" method="POST"
        ...
        <div class="d-flex justify-content-between align-items-center px-4 py-3 border-top">
            <small class="text-secondary">
                Menampilkan {{ $peserta->firstItem() }}–{{ $peserta->lastItem() }} dari {{ $peserta->total() }} peserta
            </small>
            {{ $peserta->appends(['search' => $search, 'tanggal' => request('tanggal'), 'kategori' => request('kategori')])->links('pagination::bootstrap-5') }}
        </div>
        @endif
    </div>
</div>
